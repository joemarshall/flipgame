...
public abstract class SoundStreamRunnable implements Runnable
{
	...
	public SoundStreamRunnable(int id, FileDescriptor descriptor,long offset,long length, float tempo,
			float pitchSemi) throws IOException
	{
		initDecoder(descriptor,offset,length);
	...
	private void initDecoder(FileDescriptor descriptor,long offset,long length) throws IOException
	{
		if (Build.VERSION.SDK_INT >= 16)
		{
			decoder = new MediaCodecAudioDecoder(descriptor,offset,length);
		}
		else
		{
			throw new SoundStreamRuntimeException(
