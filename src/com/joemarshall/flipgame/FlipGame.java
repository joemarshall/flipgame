...
public class FlipGame extends PApplet implements OnProgressChangedListener
{
	...
	public void onCreate(Bundle arg0)
	{
		...
		try
		{
			...
			music = new SoundStreamAudioPlayer(0, musicfd.getFileDescriptor(),
					musicfd.getStartOffset(), musicfd.getLength(), 1.0f, 0.0f);
			new Thread(music).start();
			music.setOnProgressChangedListener(this);
			music.setLoopEnd(28009000L);
			music.setLoopStart(12005000L);
	...
	private void enterPrefsScreen()
	{
		...
		m_TimeMultiplierGrid = new ButtonGrid(m_Cp5,
				GameMovement.values().length, 4, width / 10, height / 10, width
		...
		yPos += height / 10;
		yPos += height / 20;
		m_PrefsDefaultButton=m_Cp5.addButton("setDefaultPrefs").setPosition(width/10,yPos).setSize(width-width/5,height/10).setCaptionLabel("Defaults");
	...
	public void inGameThread()
	{
		...
		music.setRate((float) 0.75 * m_Prefs.m_Level1TimePerMove
				/ (float) timePerMove);
		while (alive)
		{
			if (movesLeft <= 0)
	...
	void pauseGameUntilMusic(long milliseconds)
	{
		long microseconds = milliseconds * 1000;
		while (music.getPlayedDuration() < microseconds)
		{
			pauseGame(10);
		}
	}
